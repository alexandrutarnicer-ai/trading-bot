...
- **Rails hard (`executor.py::validate_decision` + clamp in `config.py`):** geometrie SL/TP, RR≥1, SL≤5×ATR, stop zilnic -3R, risc≤1%. **Expunere ANGAJATA:** rail-ul `max_open_positions` (default 3) numara pozitii deschise **+ ordine pending** (`n_committed`), nu doar pozitii — altfel la cold-start (ledger gol) heartbeat-ul convoaca toate pietele deodata si s-ar plasa mai multe ordine stop decat limita (toate cu 0 pozitii deschise). Ordinele stop se plaseaza secvential per bara, deci contorul creste pe masura ce se plaseaza. LLM-ul propune, rails-urile dispun.
