...
# EMA — asistent VOCAL (proces STANDALONE, aditiv, optional, READ-ONLY)
setup_voice_bridge.bat                 # instaleaza faster-whisper + sounddevice + pyttsx3 (+ openwakeword)
start_voice_bridge.bat                 # dublu-click: porneste EMA (sau din UI: cardul „EMA")
python -m voice_bridge                 # manual
python -m voice_bridge.selftest        # teste offline (fara microfon/whisper/TTS/MT5)
python -m voice_bridge.voices          # diagnostic voci („de ce vorbeste EMA in engleza?")
python -m voice_bridge.miccheck        # diagnostic microfon („EMA nu ma aude") — device + nivel + STT
```
...
### `voice_bridge/` — Jarvis, asistent VOCAL (al treilea canal, aditiv, READ-ONLY)

Asistent vocal local (**„Jarvis"**, EN implicit) — al treilea canal dupa Telegram si Matrix, conectat la **acelasi `telegram_bridge.router.Router`**: `„Hey Jarvis" → Whisper (STT) → normalize → Router → TTS`. `VoiceClient.send` imita semnatura `TelegramClient.send` (rosteste in loc sa trimita) → Router-ul nu stie ca vorbeste, reutilizam 100% logica de comenzi. Proces STANDALONE, aditiv, izolat (nu importa botul/motorul in mod care le schimba starea, nu deschide a doua conexiune MT5). Doc: `docs/VOICE_BRIDGE.md`. Ghid UI: cardul „Jarvis" in Profil. **Istoric: initial numit „EMA" cu wake pe nume (STT) + RO — schimbat la Jarvis/openWakeWord/EN pt fiabilitate (STT name-match era capricios, voce RO lipsea pe Windows).**
- **READ-ONLY HARD (fara whitelist pe microfon):** `config.load_config()` forteaza `allow_writes=False` + `copilot_enabled=False`; `normalize.to_command` produce DOAR comenzi read-only (`/status /raport /piete /pauza /reia /ajutor`, `ai …`, `claude …`) — niciodata `claude!`/`/edit`. Testat: `test_no_write_commands`.
- **Trezire `wake_mode` (implicit `"openwakeword"`):** wake acustic „Hey Jarvis" cu model pre-antrenat (`wake_model="hey_jarvis"`, openWakeWord pe **onnxruntime** — tflite nesuportat pe Windows). `wake._wait_wake` ia **MAX peste scoruri** (cheia poate fi versionata `hey_jarvis_v0.1` — bug fix: inainte citea o cheie fixa → mereu 0). Fallback daca openWakeWord lipseste → **`"ptt"`** (push-to-talk, 100% sigur), NU `"name"`. Alte moduri: `"ptt"`, `"name"` (spui numele la inceput, `normalize.strip_wake_name`, STT — capricios, secundar). CLI: `py -m voice_bridge --ptt|--wake=openwakeword|--debug|--lang=en|--device N`.
- **Pauza / mut (scenariul „sunt pe Discord"):** flag `data/voice_bridge_control.json` (`control.py`), citit de bucla la fiecare iteratie → cand paused, microfonul e OPRIT complet. Trei cai: buton UI (`VoiceBridgeCard`), vocal („EMA culca-te"/„trezeste-te" — normalize `__PAUSE__`/`__RESUME__`; „pauza" fara numar de sesiune = mut EMA, cu numar = `/pauza S7`), sau Stop total. Revenire implicita din UI (`resume_by_voice=false`, ca micul sa fie chiar oprit in pauza).
- **Limba (`language`, default „en"):** forteaza limba STT (auto-detect gresea des pe fraze scurte; `config` deriva `stt_language` din `language`), alege vocea TTS, limba confirmarilor rostite (`bridge._PHRASES` en/ro) si un hint „raspunde in romana" pentru ai/claude DOAR pe `ro` (`bridge._apply_language` — Ollama raspunde implicit EN). Comenzile instant raman text RO (asa e formatat botul).
- **TTS (voce Jarvis):** `tts.Speaker` (coada + thread dedicat). Motoare, in ordinea `auto`: **Piper** (daca e configurat, OFFLINE, + efect eerie `_apply_eerie`) → **edge-tts** (`pip install edge-tts`, voci NEURALE, ex `en-GB-RyanNeural` britanic/masculin = Jarvis; ONLINE, gratuit; mp3 via MCI `_play_mp3`) → **pyttsx3** (SAPI Windows, zero-setup; `_pick_voice` alege voce pt `language`+gen din stil). Preset `voice_style` (default `jarvis` = masculin britanic; si `xalatath`/`calm`/`neutral`); stilul poate impune `edge_voice`. `speakable()` curata emoji/HTML/markdown + scurteaza (PUR). Diagnostic voci: `py -m voice_bridge.voices`.
- **STT:** `stt.Transcriber` = faster-whisper local, model implicit `base`, limba fortata din `language`. **`stt_device` default `cpu` (FIABIL pe Windows):** auto-detectul GPU alegea CUDA cand exista placa (Ollama), dar WhisperModel(device=cuda) arunca la load fara cuDNN/cuBLAS → transcribe arunca la FIECARE comanda → mesaj înselator „check microphone" desi micul mergea. `_candidates()` incearca acum device-urile in ordine cu **fallback garantat pe CPU**; `load()` = warm-up la pornire (surprinde eroarea devreme + scoate latenta). `bridge` da feedback vocal „didn't catch that" cand comanda iese goala (nu tacere). `audio.Microphone` = sounddevice + VAD energie (`vad_energy` default 0.004; `record_utterance` logheaza max-RMS cand nu aude) + `frames()` (int16 1280=80ms) pt openWakeWord — **generatorul se inchide DETERMINIST (`gen.close()`) dupa wake**, altfel inregistrarea comenzii deschidea un al doilea InputStream peste cel de wake (device ocupat → comanda goala). Diagnostic: `py -m voice_bridge.miccheck`. Deps grele importate LAZY.
- **Control din UI (`/voice-bridge/config` GET/PUT):** `wake_mode` (openwakeword/ptt/name) + `language` (en/ro) editabile din cardul Jarvis (`useVoiceConfig`/`useSaveVoiceConfig`), se aplica la repornire. PTT nu e activ decat daca il alegi explicit.
- **Config „ingheata" defaults — capcana:** `save_default_config` scrie DOAR `_STARTER_KEYS` (nu tot `VOICE_DEFAULTS`) ca schimbarile viitoare de default sa ajunga la user. Daca cineva a rulat EMA inainte de o schimbare de default, `data/voice_bridge.json` vechi suprascrie — remediu: sterge fisierul (se regenereaza minimal).
- **API + UI:** `api/routers/voice_bridge.py` (`/voice-bridge/status|start|stop|pause|resume` + `/autostart/{status,enable,disable}`, mirror al `/telegram-bridge/*`, self-contained — scrie flag-uri, nu importa pachetul). `VoiceBridgeCard.tsx` in Profil (dupa `MatrixSettings`) — Start/Stop + Pauza/Reia + toggle autostart. Hooks: `useVoice{Status,Start,Stop,Pause,Resume,AutostartStatus,AutostartEnable,AutostartDisable}`.
- **Autostart Windows (OPTIONAL, OFF by default):** `scripts/setup_autostart_voice.ps1`/`remove_autostart_voice.ps1` (task `TradingBot-VoiceEMA`, RunLevel Limited, login+60s; genereaza `voice_bridge/start_voice_auto.bat`; pur ASCII, .bat CRLF). Mirror al autostart-ului puntii Telegram. Atentie: cu autostart, EMA porneste micul la fiecare login (mod „name" asculta continuu) — de asta e OFF implicit.
- **Config:** `data/voice_bridge.json` (gitignored, auto-creat). Instalare: `setup_voice_bridge.bat`. Teste: `python -m voice_bridge.selftest` (offline: normalizator + wake-name + speakable + garantia read-only).
