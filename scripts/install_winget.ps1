...
if ($wingetExe) {
    ...
    Write-Host "  [+] winget gasit la $($wingetExe.FullName) — adaugat in PATH." -ForegroundColor Green
    exit 0
}
...
Write-Host ""
Write-Host "  Instalez winget (build OS: $buildNum, Win11: $isWin11)..." -ForegroundColor Yellow

# ── Temp dir robust — [System.IO.Path]::GetTempPath() nu depinde de env:TEMP ─
...
# ── Pe Windows 10: dependente suplimentare (pe Win11 sunt deja instalate) ────
if (-not $isWin11) {
